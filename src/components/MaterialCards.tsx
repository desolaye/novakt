    ...
    <div className="grid lg:grid-cols-2 gap-4 max-w-7xl mx-auto px-2 md:px-8">
      {MATERIALS_DATA.map((e) => (
        <Link
          key={e.id}
          to={'/' + e.id}
          className="bg-x-white grid grid-cols-2 shadow-xl rounded-sm hover:scale-95 hover:shadow-inner focus:scale-95 focus:shadow-inner transition-all h-[250px] overflow-hidden"
        >
          <div className="p-6 flex flex-col justify-between">
            <div className="flex flex-col gap-1">
              <h4 className="text-xl font-bold">{e.title}</h4>
              <p>{e.price}</p>
              {e.id === 'veneer' && <p className="text-red-800">Изучаем спрос</p>}
            </div>
